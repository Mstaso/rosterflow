...
export default function TradeCardSkeleton() {
  return (
    <div>
      ...
      <div className="flex items-center gap-2 py-3 px-4 mb-4 rounded-md border border-indigoMain/30 bg-indigoMain/5 text-muted-foreground justify-center w-full md:w-fit md:mx-0 md:justify-start">
        <Loader2 className="w-4 h-4 animate-spin text-indigoMain" />
        <div className="text-sm">
          Generating trade scenarios...
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 justify-center mb-4">
        {[1, 2].map((i) => (
          <Card
            key={i}
            className="flex flex-col h-auto overflow-hidden border-muted/30 bg-gradient-to-br from-background via-background/95 to-muted/80 md:flex-1"
          >
            ...
            <CardContent className="px-4 py-4 flex-grow flex flex-col bg-muted/60">
              {/* Cap table skeleton */}
              <div className="mb-4">
                <div className="h-4 w-36 rounded bg-muted/40 animate-pulse mb-2" />
                <div className="border border-border rounded overflow-hidden">
                  {[1, 2, 3, 4].map((j) => (
                    <div
                      key={j}
                      className={`flex justify-between py-2 px-2 ${j % 2 === 1 ? "bg-muted/40" : "bg-background"}`}
                    >
              ...
              <div className="h-4 w-28 rounded bg-muted/40 animate-pulse mb-3" />
              ...
              <div className="space-y-2">
                {[1, 2].map((j) => (
                  <div
                    key={j}
                    className="flex items-center gap-3 p-3 rounded-md border-2 border-border bg-slate-950"
                  >
