...
export async function getNBATeams() {
  try {
    return await db.teams.getAll();
  } catch (error) {
    console.warn(
      "Database not available during build, returning empty teams array"
    );
    return [];
  }
}
...
export async function getNBATeamWithRosterAndDraftPicks(teamId: number) {
  try {
    ...
    console.warn("Database not available during build, returning null");
    return null;
  }
}
